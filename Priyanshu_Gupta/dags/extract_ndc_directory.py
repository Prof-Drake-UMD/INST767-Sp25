...
from include.extract_from_apis.ExtractNdcChunk import ExtractNdcChunk
...
@dag(
    ...
    schedule = '@daily',
    catchup = True,
    tags = ['openFDA', 'ndc_directory', 'pubsub', 'ETL']
)

def ndc_directory_etl():
    """
    DAG for extracting ndc directory data from OpenFDA API, processing it in chunks,
    ...
    def poke_api( **kwargs) -> PokeReturnValue:
        """
        Description:
        Check if ndc directory api is available.
    ...
    def prepare_chunk_params(poke_result: dict, chunk_list: list) -> list[dict[str, str]]:
        """
        Description:
        Prepare parameters for each chunk, including API details.
    ...
    def fetch_raw_and_send_to_pubsub(chunk_params):
        ...
        chunk = ExtractNdcChunk(chunk_id)
        
        logger.info(f"Fetching chunk {chunk_id}")
        json_data = chunk.fetch_ndc_chunk(url=url, api_key=api_key, record_limit=1000)
        logger.info(f'Complete data fetching for chunk {chunk_id}.')

        object_name=f'{raw_prefix}/ndc-chunk-{chunk_id}.json'
