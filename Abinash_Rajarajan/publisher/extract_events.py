...
def extract_events(request):
    ...
    key        = get_secret("ticketmaster-key")
    url        = f"https://app.ticketmaster.com/discovery/v2/events.json?size=100&apikey={key}"
    data       = requests.get(url).json()

    store_json_to_gcs(data, bucket, "raw", "events-tickermaster")
    msg = {"bucket":bucket, "path":"raw/events-tickermaster.json", "domain":"events"}
