"""
Simple pipeline runner script that works with direct imports.
Save this as run_pipeline.py in the Yuting_Shen directory.
"""
...
# Import directly from src packages
...
def main():
    ...
    try:
        ...
        trends_config = {
            ...
            'request_interval': 60,  # 30 seconds between requests
            'retries': 3,
            'backoff_factor': 2,
            'session_cooldown': 60
        }
        ...
        if hasattr(pipeline, 'events_data'):
            logger.info("Data counts:")
            logger.info(f"- Events: {len(pipeline.events_data)}")
            logger.info(f"- Teams: {len(pipeline.teams_data)}")
            logger.info(f"- YouTube Videos: {len(pipeline.videos_data)}")
            logger.info(f"- Video Metrics: {len(pipeline.video_metrics_data)}")
            logger.info(f"- Search Trends: {len(pipeline.search_trends_data)}")
            logger.info(f"- Integrated Analyses: {len(pipeline.integrated_analysis_data)}")
